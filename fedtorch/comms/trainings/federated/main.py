...
def train_and_validate_federated(client):
    ...
    for n_c in range(client.args.num_comms):
        ...
        client.args.comp_time.append(0.0)
        ...
        if client.args.graph.rank in online_clients_server:
            if  client.args.federated_type == 'scaffold':
                ...
                st = time.time()
                client.model_server, client.model_server_ag = distribute_model_server_aq(client.model_server,
                                                                                         client.model_server_ag,
                                                                                         online_clients_group,
                                                                                         src=0)
                client.args.comm_time[-1] += time.time() - st
            else:
            ...
            if client.args.federated_type == 'fedaq':
                client.model_ag.load_state_dict(client.model_server_ag.state_dict())
            ...
            if client.args.graph.rank in online_clients:
                ...
                while not is_sync:
                    for _input, _target in client.train_loader:
                        ...
                        if client.args.federated_type == 'fedaq':
                            client.optimizer.store_current()
                            beta = client.optimizer.get_beta()
                            # compute the middle parameter by coupling
                            for client_param, client_ag_param in zip(client.model.parameters(), client.model_ag.parameters()):
                                client_param.data = 1/beta * client_param.data + (1 - 1/beta) * client_ag_param.data
                        ...
                        if client.args.federated_type == 'fedaq':
                            client.optimizer.step(apply_lr=True)
                        else:
                ...
                client.args.comp_time[-1] += time.time() - st_comp
            ...
            if client.args.federated_type == 'fedgate':
                ...
                client.model_server, client.model_server_ag = fedaq_aggregation(client.args, client.model_server, client.model,
                                                                                client.model_server_ag, client.model_ag,
                                                                                online_clients_group, online_clients,
                                                                                client.optimizer, lr, local_steps)
            else:
